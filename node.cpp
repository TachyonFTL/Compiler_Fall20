...
void Node::set_oprand(struct Operand* oprd){
  op = oprd;
}

struct Operand* Node::get_oprand(){
  return op;
}
...
void node_set_oprand(struct Node *n, struct Operand* oprd){
  n->set_oprand(oprd);
}

struct Operand* node_get_oprand(struct Node *n){
  return n->get_oprand();
}
...
void error_at_node(struct Node *node, const char *msg) {
  struct SourceInfo source_pos = node_get_source_info(node);
